 ...
 * 4)要String后端给了[],{}等类型返回""
 * https://blog.csdn.net/a850125665/article/details/79214797
 */
public class MGson {
    ...
    public static Gson newGson() {
        ...
        try {
            ...
            //注册反射对象的处理器
            gsonBuilder.registerTypeAdapterFactory(new ReflectiveTypeAdapterFactory(new ConstructorConstructor(val,true,new ArrayList<>()), FieldNamingPolicy.IDENTITY, Excluder.DEFAULT));
            //注册集合的处理器
            gsonBuilder.registerTypeAdapterFactory(new CollectionTypeAdapterFactory(new ConstructorConstructor(val,true,new ArrayList<>())));
        } catch (Exception e) {
            e.printStackTrace();
        }
