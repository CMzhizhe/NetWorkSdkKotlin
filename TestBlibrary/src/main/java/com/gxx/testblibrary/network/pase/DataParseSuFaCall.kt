package com.gxx.testblibrary.network.pase
...
import com.gxx.testblibrary.model.BaseBean
...
open class DataParseSuFaCall<T> : AbsRequestResultImpl() {
    ...
    override fun onRequestSuccess(
        ...
        if (targetElement!=null){
            var result:Any?=null
            try {
                result = mGson.fromJson<BaseBean>(targetElement.toString(),object : TypeToken<T>() {}.type)
            } catch (e: Exception) {
                e.printStackTrace()
                //处理解析异常
